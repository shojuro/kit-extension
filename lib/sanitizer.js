...
class InputSanitizer {
  ...
  sanitize(input, profile = 'strict') {
    try {
      ...
      // Return empty string on error (safe default)
      return '';
    }
  }
  ...
  escapeHtml(text) {
    ...
    // This is safe - we're reading the innerHTML after setting textContent
    // which automatically escapes the HTML
  ...
  unescapeHtml(text) {
    ...
    const sanitized = this.sanitize(text, 'strict');
  ...
  sanitizeWithDOMPurify(text, profile) {
    try {
      ...
      this.logSecurityEvent('DOMPURIFY_ERROR', { 
        error: error.message,
        profile: profile
      });
      // Fallback to manual sanitization
      return this.sanitizeWithFallback(text, profile);
    }
  }
  ...
  sanitizeWithFallback(text, profile) {
    ...
    // Return the cleaned HTML - this is controlled innerHTML usage after sanitization
    return temp.innerHTML;
    } catch (error) {
      this.logSecurityEvent('FALLBACK_SANITIZATION_ERROR', { 
        error: error.message,
        profile: profile
      });
      // Return escaped text as safest fallback
